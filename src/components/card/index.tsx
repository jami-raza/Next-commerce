...
export function ProductCard(prop:IProductCard) {
  ...
  const cart = useAppSelector((state) => state.cartReducer);
    ...
    return (
      <Card className="w-[100%] max-w-[23%] mt-4 max-md:max-w-[48%] max-sm:max-w-[100%]" key={prop._id}>
          <Link href={`/product/${prop._id}`} passHref>
          <CardHeader className="p-0">
             <Image src={prop.image} alt="Product" width={1000} height={800}  className="h-80 w-[100%] object-cover rounded-t-lg"	  />
          </CardHeader>
              </Link>
...
{/* <p className="text-lg font-bold text-black truncate block capitalize"></p> */}
            ...
            <h3 className="font-bold">Rs. {prop.price} </h3>
